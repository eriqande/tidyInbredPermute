...
#still an issue here, could fix by getting year from teh spawn date column if needed
```
...
#Need to seperate by hatcheries -- Warm springs and coyote valley
```{r}
library(lubridate)
...
#switched it to "interm1" instead of "interm3" the code above seemed unnecessary with the 2007-2020 dataset
```{r}
#Warm Springs
...
```{r}
(sum(permy_dsn_CV$quellergt > 0.5) / 100) / nrow(obs_dsn_WS)
```
...
```{r}
sum(obs_dsn_CV$quellergt > 0.5)
```
So, we saw one of those.

How about over 0.4?
```{r}
sum(obs_dsn_CV$quellergt > 0.4)
```
...
RP_CV <- rbinom(n = 1e5, size = nrow(obs_dsn_CV), prob =0.00098161)
hist(RP)
```
